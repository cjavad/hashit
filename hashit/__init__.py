    ...
    if hasattr(hashit, "name"):
        ...
        if len(tup.certain) >= 1 and not (hashit.name in tup.certain or hashit.name in tup.maybe):
            ...
            if len(tup.maybe) - 1 >= c_index:  
                # choose maybe
                hashit = hashlib.new(tup.maybe[c_index])
...
def hashFile(filename, hasher, memory_opt=True):
    if memory_opt:
        return hashIter(blockIter(open(filename, "rb")), hasher, True)
    else:
        # dont use memory optimatation
        return hashlib.new(hasher.name, open(filename, "rb").read()).hexdigest()
...
def check(path, hashit, useColors=False,  be_quiet=False, detectHash=True, sfv=False):
    """Will read an file which have a SFV compatible hash table or a standard one and verify the files checksum"""
    ...
    try:
        if detectHash:
            ...
            # get hash from line
            hash1 = first_line.replace("\n", "").replace("\0", "").split(" ")[hash_index]
            # get new hasher
    ...
    for line in x_reader():
        ...
        if os.path.exists(filename):
            ...
            current_hash = hashlib.new(hashit.name, open(filename, "rb").read()).hexdigest()
